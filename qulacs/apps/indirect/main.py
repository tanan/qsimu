...
def cost(random_list):
    ...
    if config["gate"]["type"] == "indirect_by_none":
        ...
        ansatz = AnsatzIndirectByXY(
            n_qubit, random_list, config["depth"], config["gate"]["is_bn_random"]
        )
...
def output(param_history, cost_history, iter_history):
    ...
    if config["gate"]["type"] == "direct":
        param_path = "results/%squbit_%s_%s_param.txt" % (
            n_qubit,
            config["gate"]["type"],
            config["depth"],
        )
        cost_path = "results/%squbit_%s_%s_cost.txt" % (
            n_qubit,
            config["gate"]["type"],
            config["depth"],
        )
        iter_path = "results/%squbit_%s_%s_iter.txt" % (
            n_qubit,
            config["gate"]["type"],
            config["depth"],
        )
    else:
        param_path = "results/%squbit_%s_%s_%s_%s_%s_%s_param_%s.txt" % (
            n_qubit,
            config["gate"]["type"],
            config["depth"],
            config["max_time"],
            config["gate"]["is_r_random"],
            config["gate"]["is_cn_random"],
            config["gate"]["is_bn_random"],
            tstr,
        )
        cost_path = "results/%squbit_%s_%s_%s_%s_%s_%s_cost_%s.txt" % (
            n_qubit,
            config["gate"]["type"],
            config["depth"],
            config["max_time"],
        ...
        iter_path = "results/%squbit_%s_%s_%s_%s_%s_%s_iter_%s.txt" % (
            n_qubit,
            config["gate"]["type"],
            config["depth"],
            config["max_time"],
...
def run():
    ...
    if config["gate"]["type"] == "direct":
        opt = minimize(cost, init_random_list, method="BFGS", callback=record)
    else:
        opt = minimize(
            cost,
            init_random_list,
