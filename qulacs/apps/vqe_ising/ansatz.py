...
class AnsatzIndirectByXYZ:
    ...
    def create_ansatz(self, random_list):
        ...
        for d in range(self.depth):
            circuit.add_gate(
                merge(
                    RX(0, random_list[self.depth + (self.gate_set * d)]),
                    RY(0, random_list[self.depth + (self.gate_set * d) + 1]),
                )
            )
            # TODO: cnをrandom化する
            circuit.add_gate(self.create_hamiltonian_gate(random_list[d]))
    ...
    def create_ansatz(self, random_list):
        ...
        for d in range(self.depth):
            ...
            if self.bn["type"] == "random":
                ...
                circuit.add_gate(
                    merge(
                        RY(0, random_list[self.depth + (self.gate_set * d)]),
                        RZ(0, random_list[self.depth + (self.gate_set * d) + 1]),
                    )
                )
                circuit.add_gate(
                    merge(
                        RY(1, random_list[self.depth + (self.gate_set * d) + 2]),
                        RZ(1, random_list[self.depth + (self.gate_set * d) + 3]),
                    )
                )
                circuit.add_gate(self.create_hamiltonian_gate(random_list[d]))
...
class AnsatzIndirectByNone:
    ...
    def create_ansatz(self, random_list):
        ...
        for d in range(self.depth):
            ...
            circuit.add_gate(
                merge(
                    RY(0, random_list[self.gate_set * d + self.depth]),
                    RZ(0, random_list[self.gate_set * d + (self.depth + 1)]),
                )
            )
            circuit.add_gate(
                merge(
                    RY(1, random_list[self.gate_set * d + (self.depth + 2)]),
                    RZ(1, random_list[self.gate_set * d + (self.depth + 3)]),
                )
            )
