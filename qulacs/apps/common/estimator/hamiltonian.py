...
from common.hamiltonian import Coefficients, Hamiltonian, HamiltonianModel, HamiltonianModelError
    ...
    def value(self) -> np.ndarray:
        ...
        for x in self.xs:
            ...
            for obs in self.obs:
                ...
                self.hamiltonian.circuit(300 + (4 * (x + 1))).update_quantum_state(state)
...
def create_hamiltonian_estimator(nqubit: int, obs: Observables, coef: Coefficients, hamiltonianModel: HamiltonianModel, xs) -> Hamiltonian:
    if hamiltonianModel == HamiltonianModel.TRANSVERSE_ISING:
        hamiltonian = create_transverse_ising_hamiltonian_generator(nqubit, coef[0], coef[1], hamiltonianModel)
    else:
