  ...
  def create(self, current_time, start_time, end_time, cost_history, param_history, iter_history):
    if self.config['gate']['type'] == 'direct':
      ...
      job = Job(
        ...
        self.config['gate']['time']['max_val'] if 'max_val' in self.config['gate']['time'] else None,
        self.config['gate']['time']['min_val'] if 'min_val' in self.config['gate']['time'] else None,
        str(self.config['gate']['time']['value']) if 'value' in self.config['gate']['time'] else None,
        str(cost_history[-1]),
        str(param_history[-1]),
        str(iter_history[-1]),
        str(cost_history),
